...
import CheckMessages from '../components/Logic/CheckMessages';
...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    ...
    if (keypairCheck) {
      ...
      setKeypair(solanaWeb3.Keypair.fromSecretKey(new Uint8Array(Object.values(JSON.parse(keypairCheck)._keypair.secretKey))));
      setPubkey(new solanaWeb3.PublicKey(Object.values(JSON.parse(keypairCheck)._keypair.publicKey)));
      setTimeout(async () => {
        setConversations(await GetConversations(solanaWeb3.Keypair.fromSecretKey(new Uint8Array(Object.values(JSON.parse(keypairCheck)._keypair.secretKey)))));
      }, 5);
      setInterval(async () => {
        try{
          console.log(solanaWeb3.Keypair.fromSecretKey(new Uint8Array(Object.values(JSON.parse(keypairCheck)._keypair.secretKey))));
          setConversations(await GetConversations(solanaWeb3.Keypair.fromSecretKey(new Uint8Array(Object.values(JSON.parse(keypairCheck)._keypair.secretKey)))));
        } catch (err) {
          console.error(err);
        }
  ...
  <>
    ...
    <meta name="description" content="Privacy focused, secure, and encrypted messaging." />
    ...
    {/* <!-- Android --> */}
    <link rel="shortcut icon" sizes="196x196" href="/static/icon/logo196.png"/>
