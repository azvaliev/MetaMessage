...
export default function Conversation(props) {
    ...
    return (
        <View style={{height: '100vh', backgroundColor: 'black'}}>
                ...
                // onClick={HandleCopyAddress(address)}
                    style={{
                       ...
                       lineHeight: '2.25rem'
                        ...
                        {attemptShortenPubkey(address, false, props.mobile)}
                </Text>
            </View>
            </>
            :
            <>
                <div className="flex -mb-18 ml-4 font-extrabold z-20 text-7xl text-blue-600" onClick={closeConversation}>&#x226A;</div>
            ...
            <KeyboardAvoidingView style={{height: height}}  nativeID='main-conversation'>
                <ScrollView 
                ref={scrollRef}
                style={{
                    marginLeft: '2%',
                    ...
                    {activeConversation.map((conversation, i) => {
                        if (activeConversation.length === i + 1) {
                            return (<Message 
                                ...
                                message={conversation.message} showDate={true} date={conversation.date}/>)
                        } else {
                            return (<Message 
                                from={conversation.from === address ? true : false} 
                                message={conversation.message} showDate={false} date={conversation.date}/>)
                        }
                ...
                {props.mobile?
                <NativeComposeMessage message={messageContents} handleTypingMessage={handleTypingMessage} handleSendMessage={handleSendMessage} onBlur={onBlur} onFocus={onFocus}/>
                :
                <DesktopCompose bottom='bottom-0' message={messageContents} handleTypingMessage={handleTypingMessage} handleSendMessage={handleSendMessage} />
                }
